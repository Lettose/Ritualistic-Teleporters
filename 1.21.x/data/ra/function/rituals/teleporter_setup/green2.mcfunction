...
execute at @s run tag @e[type=marker,distance=0..1,sort=nearest,limit=1] add green2
...
execute as @s unless entity @e[tag=green2,distance=1..] run kill @e[type=item,distance=0..1,nbt={Item:{id:"minecraft:echo_shard",count:1}},limit=1,sort=nearest]

execute at @s at @e[tag=green2,distance=1..] run function ra:rituals/particles/break
execute at @s run kill @e[type=marker,tag=green2,distance=1..]

execute as @s run kill @e[type=item,distance=0..1,nbt={Item:{id:"minecraft:green_dye",count:1}},limit=1,sort=nearest]