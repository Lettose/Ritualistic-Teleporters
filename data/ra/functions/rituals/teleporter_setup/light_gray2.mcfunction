...
execute at @s run tag @e[type=minecraft:armor_stand,distance=0..1,sort=nearest,limit=1] add light_gray2
...
execute as @s unless entity @e[tag=light_gray2,distance=1..] run kill @e[type=item,distance=0..1,nbt={Item:{id:"minecraft:echo_shard",Count:1b}},limit=1,sort=nearest]

execute at @s at @e[tag=light_gray2,distance=1..] run function ra:rituals/particles/break
execute at @s run kill @e[type=armor_stand,tag=light_gray2,distance=1..]

execute as @s run kill @e[type=item,distance=0..1,nbt={Item:{id:"minecraft:light_gray_dye",Count:1b}},limit=1,sort=nearest]