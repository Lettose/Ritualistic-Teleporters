...
execute as @s unless entity @e[tag=light_gray2,distance=1..] run kill @e[type=item,distance=0..1,nbt={Item:{id:"minecraft:echo_shard",count:1}},limit=1,sort=nearest]
...
execute as @s run kill @e[type=item,distance=0..1,nbt={Item:{id:"minecraft:light_gray_dye",count:1}},limit=1,sort=nearest]