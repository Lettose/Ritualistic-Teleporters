...
execute as @s unless entity @e[tag=gray2,distance=1..] run kill @e[type=item,distance=0..1,nbt={Item:{id:"minecraft:echo_shard",count:1}},limit=1,sort=nearest]
...
execute as @s run kill @e[type=item,distance=0..1,nbt={Item:{id:"minecraft:gray_dye",count:1}},limit=1,sort=nearest]